...
type (
	DB struct {
		...
		durabilityFailureStreak  int
		lastSyncLatency          time.Duration
		metrics                  metrics
		commitLatency            commitLatencyHistogram
		segmentReads             sync.Map
		valueLog                 *valueLog
		pendingDurabilityFiles   []*DataFile
		groupCommit              *groupCommitCoordinator
		adaptiveSync             *adaptiveSyncLoop
		lifecycle                *lifecyclePlacement
		autonomous               *autonomousState
		configuredCompactionMode CompactionPolicyMode
		configuredLifecycle      bool
		compactionRecommendation *compactionRecommendationState
	}
...
func Open(opt Options) (*DB, error) {
	...
	if err := recoverMergeManifest(opt); err != nil {
		return nil, fmt.Errorf("merge recovery error: %s", err)
	}
	db := &DB{
		opt:                      opt,
		...
		lastSyncAt:               time.Now(),
		configuredCompactionMode: opt.Compaction.Mode,
		configuredLifecycle:      opt.KVSeparation.LifecycleEnable,
	}
	//判断文件夹在不在,不在就创建
	...
	if opt.KVSeparation.LifecycleEnable && !opt.KVSeparation.Enable {
		return nil, errors.New("lifecycle KV separation requires KV separation to be enabled")
	}
	if opt.KVSeparation.Enable {
		valueLog, err := openValueLog(opt.Dir, opt.KVSeparation.ValueLogSegmentSize)
		if err != nil {
			return nil, err
		}
		db.valueLog = valueLog
		if opt.KVSeparation.LifecycleEnable {
	...
	if db.valueLog != nil {
		if err := db.recoverValueLogGC(); err != nil {
			return nil, fmt.Errorf("value-log GC recovery error: %s", err)
		}
	}
	if opt.Autonomous.EnableRecommendations {
		db.autonomous = newAutonomousState(opt.Autonomous)
	}
	if opt.Compaction.EnableSLORecommendation {
		db.compactionRecommendation = newCompactionRecommendationState(opt.Compaction)
	}
	switch db.effectiveSyncPolicy() {
	case SyncPolicyGroupCommit:
		db.groupCommit = newGroupCommitCoordinator(db, opt.SyncPolicy)
	case SyncPolicyAdaptive:
		db.adaptiveSync = newAdaptiveSyncLoop(db, opt.SyncPolicy.AdaptiveMaxDelay)
	}
...
func (db *DB) Close() error {
	...
	if db.groupCommit != nil {
		syncErr = db.groupCommit.close()
	} else if db.adaptiveSync != nil {
		syncErr = db.adaptiveSync.close()
	} else if db.effectiveSyncPolicy() != SyncPolicyNone {
		syncErr = db.syncDurabilityResources()
	}

	db.mu.Lock()
	var retainedCloseErr error
	for _, dataFile := range db.pendingDurabilityFiles {
		if err := dataFile.Close(); err != nil && retainedCloseErr == nil {
			retainedCloseErr = err
		}
	}
...
func (db *DB) Merge() error {
	return db.mergeWithBudget(0)
}

func (db *DB) MergeWithBudget(maxBytes int64) error {
	if maxBytes <= 0 {
		return errors.New("merge budget must be positive")
	}
	return db.mergeWithBudget(maxBytes)
}
...
func (db *DB) mergeWithBudget(maxBytes int64) error {
	...
	if maxBytes > 0 {
		segments, err := db.mergeSegments(pendingMergeFIds)
		if err != nil {
			return err
		}
		pendingMergeFIds = budgetMergeFileIDs(segments, maxBytes)
		if len(pendingMergeFIds) == 0 {
			return nil
		}
	}
	...
	for _, pendingMergeFId := range pendingMergeFIds {
		...
		firstTargetFileID := db.MaxFileID + 1
		if err := db.writeMergeManifest(mergeManifest{Phase: mergeManifestPrepared, SourceFileID: int64(pendingMergeFId), FirstTargetFileID: firstTargetFileID, LastTargetFileID: firstTargetFileID - 1}); err != nil {
			f.rwManager.Close()
			return err
		}
		if err := db.injectSemanticFault(FaultPointMergePrepared); err != nil {
			f.rwManager.Close()
			return err
		}
		...
		lastTargetFileID := db.MaxFileID
		if len(pendingMergeEntries) == 0 {
			lastTargetFileID = firstTargetFileID - 1
		}
		if err := db.syncDurabilityResources(); err != nil {
			f.rwManager.Close()
			return err
		}
		if err := db.writeMergeManifest(mergeManifest{Phase: mergeManifestInstalled, SourceFileID: int64(pendingMergeFId), FirstTargetFileID: firstTargetFileID, LastTargetFileID: lastTargetFileID}); err != nil {
			f.rwManager.Close()
			return err
		}
		if err := db.injectSemanticFault(FaultPointMergeInstalled); err != nil {
			f.rwManager.Close()
			return err
		}

		f.rwManager.Close()
		if err := db.injectSemanticFault(FaultPointMergeBeforeSourceRemove); err != nil {
			return err
		}
		if err := os.Remove(db.getDataPath(int64(pendingMergeFId))); err != nil {
			return fmt.Errorf("when merge err: %s", err)
		}
		if err := syncDirectory(db.opt.Dir); err != nil {
			return err
		}
		if err := db.clearMergeManifest(); err != nil {
			return err
		}
	}
	...
	for _, record := range unconfirmedRecords {
		...
		if _, ok := db.committedTxIds[record.H.Meta.TxID]; ok {
			...
			if isBPTreeIndexedDataStructure(record.H.Meta.Ds) {
				...
				// TODO: when EntryIdxMode is HintBPTSparseIdxMode, rebuild via BPTree.
				// 如果 是稀疏索引模式，那么 就构建
				if db.opt.EntryIdxMode == HintBPTSparseIdxMode {
